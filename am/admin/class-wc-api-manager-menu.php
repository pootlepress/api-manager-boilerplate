...
class API_Manager_Example_MENU {
	...
	public function add_menu() {
		...
		$page = add_options_page( __( AME()->settings_menu_title, AME()->text_domain ), __( AME()->settings_menu_title, AME()->text_domain ),
						'manage_options', AME()->activation_tab_key, array( $this, 'config_page')
		);
	...
	public function config_page() {
		...
		$settings_tabs = array( AME()->activation_tab_key => __( AME()->menu_tab_activation_title, AME()->text_domain ), AME()->deactivation_tab_key => __( AME()->menu_tab_deactivation_title, AME()->text_domain ) );
		$current_tab = isset( $_GET[ 'tab' ] ) ? $_GET[ 'tab' ] : AME()->activation_tab_key;
		$tab = isset( $_GET[ 'tab' ] ) ? $_GET[ 'tab' ] : AME()->activation_tab_key;
		?>
		<div class='wrap'>
			<?php screen_icon(); ?>
			<h2><?php _e( AME()->settings_title, AME()->text_domain ); ?></h2>

			<h2 class="nav-tab-wrapper">
			<?php
				foreach ( $settings_tabs as $tab_page => $tab_name ) {
					$active_tab = $current_tab == $tab_page ? 'nav-tab-active' : '';
					echo '<a class="nav-tab ' . $active_tab . '" href="?page=' . AME()->activation_tab_key . '&tab=' . $tab_page . '">' . $tab_name . '</a>';
				}
			?>
			</h2>
				<form action='options.php' method='post'>
					<div class="main">
				<?php
					if( $tab == AME()->activation_tab_key ) {
							settings_fields( AME()->data_key );
							do_settings_sections( AME()->activation_tab_key );
							submit_button( __( 'Save Changes', AME()->text_domain ) );
					} else {
							settings_fields( AME()->deactivate_checkbox );
							do_settings_sections( AME()->deactivation_tab_key );
							submit_button( __( 'Save Changes', AME()->text_domain ) );
					}
				?>
					</div>
	...
	public function load_settings() {
		...
		register_setting( AME()->data_key, AME()->data_key, array( $this, 'validate_options' ) );

		// API Key
		add_settings_section( 'api_key', __( 'API License Activation', AME()->text_domain ), array( $this, 'wc_am_api_key_text' ), AME()->activation_tab_key );
		add_settings_field( 'status', __( 'API License Key Status', AME()->text_domain ), array( $this, 'wc_am_api_key_status' ), AME()->activation_tab_key, 'api_key' );
		add_settings_field( 'api_key', __( 'API License Key', AME()->text_domain ), array( $this, 'wc_am_api_key_field' ), AME()->activation_tab_key, 'api_key' );
		add_settings_field( AME()->activation_email, __( 'API License email', AME()->text_domain ), array( $this, 'wc_am_api_email_field' ), AME()->activation_tab_key, 'api_key' );

		// Activation settings
		register_setting( AME()->deactivate_checkbox, AME()->deactivate_checkbox, array( $this, 'wc_am_license_key_deactivation' ) );
		add_settings_section( 'deactivate_button', __( 'API License Deactivation', AME()->text_domain ), array( $this, 'wc_am_deactivate_text' ), AME()->deactivation_tab_key );
		add_settings_field( 'deactivate_button', __( 'Deactivate API License Key', AME()->text_domain ), array( $this, 'wc_am_deactivate_textarea' ), AME()->deactivation_tab_key, 'deactivate_button' );
	...
	public function wc_am_api_key_field() {
		...
		echo "<input id='api_key' name='" . AME()->data_key . "[" . 'api_key' ."]' size='25' type='text' value='" . AME()->options['api_key'] . "' />";
		if ( AME()->options['api_key'] ) {
	...
	public function wc_am_api_email_field() {
		...
		echo "<input id='activation_email' name='" . AME()->data_key . "[" . AME()->activation_email ."]' size='25' type='text' value='" . AME()->options[AME()->activation_email] . "' />";
		if ( AME()->options[AME()->activation_email] ) {
			echo "<span class='icon-pos'><img src='" . AME()->plugin_url() . "am/assets/images/complete.png' title='' style='padding-bottom: 4px; vertical-align: middle; margin-right:3px;' /></span>";
		} else {
			echo "<span class='icon-pos'><img src='" . AME()->plugin_url() . "am/assets/images/warn.png' title='' style='padding-bottom: 4px; vertical-align: middle; margin-right:3px;' /></span>";
		}
	...
	public function validate_options( $input ) {
		...
		$options[AME()->activation_email] = trim( $input[AME()->activation_email] );

		/**
		  * Plugin Activation
		  */
		$api_email = trim( $input[AME()->activation_email] );
		$api_key = trim( $input['api_key'] );

		$activation_status = get_option( AME()->activated_key );
		$checkbox_status = get_option( AME()->deactivate_checkbox );

		$current_api_key = AME()->options['api_key'];

		// Should match the settings_fields() value
		if ( $_REQUEST['option_page'] != AME()->deactivate_checkbox ) {
			...
			if ( $activation_status == 'Deactivated' || $activation_status == '' || $api_key == '' || $api_email == '' || $checkbox_status == 'on' || $current_api_key != $api_key  ) {
				...
				if ( $activate_results['activated'] === true ) {
					...
					update_option( AME()->activated_key, 'Activated' );
					update_option( AME()->deactivate_checkbox, 'off' );
				}
				...
				if ( $activate_results == false ) {
					...
					$options['api_key'] = '';
					$options[AME()->activation_email] = '';
					update_option( AME()->options[AME()->activated_key], 'Deactivated' );
				}
	...
	public function license_key_status() {
		...
		$args = array(
			'email' => AME()->options[AME()->activation_email],
			'licence_key' => AME()->options['api_key'],
			);
	...
	public function wc_am_license_key_deactivation( $input ) {
		...
		if ( $options == 'on' && $activation_status == 'Activated' && AME()->options['api_key'] != '' && AME()->options[AME()->activation_email] != '' ) {
			...
			if ( $activate_results['deactivated'] === true ) {
				$update = array(
					'api_key' => '',
					AME()->activation_email => ''
					);

				$merge_options = array_merge( AME()->options, $update );

				update_option( AME()->data_key, $merge_options );

				update_option( AME()->activated_key, 'Deactivated' );
	...
	public function wc_am_deactivate_textarea() {
		...
		echo '<input type="checkbox" id="' . AME()->deactivate_checkbox . '" name="' . AME()->deactivate_checkbox . '" value="on"';
		echo checked( get_option( AME()->deactivate_checkbox ), 'on' );
		echo '/>';
	...
	public function css_scripts() {
		...
		wp_register_style( AME()->data_key . '-css', AME()->plugin_url() . 'am/assets/css/admin-settings.css', array(), AME()->version, 'all');
		wp_enqueue_style( AME()->data_key . '-css' );
	}
